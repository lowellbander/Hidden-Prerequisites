...
serialRank = function(nmatrix) {
  # nmatrix is the normalized comparison matrix
...
leastSquaresRanking = function(preferenceMatrix) {
  # preferenceMatrix acts as graph G
  ...
  #since k <= n(n-1)/2 we remove the unused rows in y and X
  ...
  #least square solution : X^T X r = X^T y
  
  XTX = t(X) %*% X #X^T X
  XTy = t(X) %*% y #X^T y
  
  # make last row of X = all 1s and last entry of y = 0 since the equations can be dependent 
  for (i in 1:n) {
    XTX[n, i] <- 1;
  }
